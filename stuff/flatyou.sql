...
CREATE TABLE `apartments` (
  ...
  `codee` char(8) COLLATE utf8_bin NOT NULL,
  ...
  `typology` enum('monolocale','appartamento','casa','bifamiliare','studentato','altro') COLLATE utf8_bin NOT NULL,
...
CREATE TABLE `beds` (
  ...
  `typology` enum('s','m','ds','dm') COLLATE utf8_bin NOT NULL DEFAULT 's',
  `state` enum('o','lo','lp') COLLATE utf8_bin NOT NULL DEFAULT 'lp',
  PRIMARY KEY (`id`),
  KEY `room_id` (`room_id`),
  KEY `state` (`state`),
